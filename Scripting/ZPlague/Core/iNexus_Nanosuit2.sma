...
#define V_CONC "models/zombie_plague/v_conc.mdl"
#define P_CONC "models/zombie_plague/p_conc.mdl"
#define W_CONC "models/zombie_plague/w_conc.mdl"
...
new cl_nn_weapon[33], bool:cl_nn_online[33]
new Float:cl_nn_damage_time[33], cl_nn_has[33], Float:g_nn_energy[33]
new NanoSpdMode:cl_nn_sp_status[33]
...
new nd_menu[33], nd_hud_sync, nd_hud_sync2, 
nd_ent_monitor, nd_msg_ammox, g_screen, g_item1, g_item2
...
g_item1 = zp_items_register("Nanosuit \rBooster +50.0% \y(End Of Map)", 30)
g_item2 = zp_items_register("EMI Bomb \y(Nanosuit Down)", 15)
...
precache_model(V_CONC)
precache_model(P_CONC)
precache_model(W_CONC)
}
...
client_cmd(id, "bind mwheelup get_energy")
client_cmd(id, "bind mwheeldown take_energy")
}
...
{
native_set_user_nanosuit(i, 1)
}
return PLUGIN_CONTINUE
}

public zp_user_infected_post(id, attacker, nemesis)
{
if(zp_core_is_zombie(id))				
...
}else native_set_user_nanosuit(id, 0)
}
...
return;  
static text[200]
format(text,199,"\yCryNet Systems \rSuit")
nd_menu[id] = menu_create(text, "nanosuit_menu_choose")

format(text,199,"Maximum Strength \d(High Damage)")
...
if(g_had_emi[id])
{
set_pev(id, pev_viewmodel2, V_CONC)
set_pev(id, pev_weaponmodel2, P_CONC)
}		
}
...
{	
static hud[200]
formatex(hud, 199, "Current Skill:",id)
switch (cl_nn_mode[id])
{
case NANO_STABILITY: formatex(hud, 199, "%s Recovery ^n",hud)	
case NANO_STREN: formatex(hud, 199, "%s High Damage ^n",hud)
case NANO_ARMOR: formatex(hud, 199, "%s Defense ^n",hud)
case NANO_SPEED: formatex(hud, 199, "%s Quickness ^n",hud)
case NANO_CLOAK: formatex(hud, 199, "%s Invisiblity ^n",hud)
case NANO_ARMOR2: formatex(hud, 199, "%s Defense ^n",hud)
}
formatex(hud, 199, "%sEnergy / (%d%%)", hud, floatround(cl_nn_energy[id] / energy * 100))
if (cl_nn_mode[id] == NANO_STREN)
{
formatex(hud, 199, "%s^nController / (%d%%)", hud,  floatround(cl_nn_controlling[id] / energy * 100))
}
else if (cl_nn_mode[id] == NANO_SPEED)
{
...
else if(cl_nn_mode[id] == NANO_STABILITY)
{
if(cl_nn_energy2[id])
{
...
{
formatex(hud, 199, "%s^nYou Don't Have Any Boosts!", hud)
}
}
}
...
energy2 = 3.3 // Regenerate
else
energy2 = 3.0 // Regenerate
...
remove_task(id + TASK_WARNED)
if (task_exists(id + TASK_CONCED))
remove_task(id + TASK_CONCED)
...
new menu = menu_create("\y[NS2] Bind Menu", "Bind_Menu1") 
...
message_end()
engfunc(EngFunc_SetModel, ent, W_CONC)
...
write_short(3<<14)
message_end()
native_set_user_nanosuit(victim, 0)
...
fm_set_user_health(ID_SCREENHUD, get_user_health(ID_SCREENHUD) - 5);  
...
fm_set_user_health(ID_SCREENHUD, hp - 5);  
...
native_set_user_nanosuit(ID_RETURNSUIT, 1)
} 
