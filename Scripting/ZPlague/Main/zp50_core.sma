...
g_Forwards[FW_USER_RUNNING] = CreateMultiForward("RuningTime", ET_IGNORE)
g_Forwards[FW_USER_RUNNING_PLAYER] = CreateMultiForward("RuningTime_Player", ET_IGNORE, FP_CELL)
...
set_task(1.0, "ZEVO_RunningTime", TASK_CORE,_,_, "b")
g_MaxPlayers = get_maxplayers()

server_cmd("mp_footsteps 0")
server_cmd("cl_shadows 0")
server_cmd("sv_shadows 0")
}
...
{
ExecuteForward(g_Forwards[FW_USER_RUNNING], g_ForwardResult)
...
ExecuteForward(g_Forwards[FW_USER_RUNNING_PLAYER], g_ForwardResult, i)
}
}
...
if (flag_get(g_IsZombie, attacker) && !flag_get(g_IsZombie, victim))
{
if(flag_get(g_IsLastHuman, victim))
{	
