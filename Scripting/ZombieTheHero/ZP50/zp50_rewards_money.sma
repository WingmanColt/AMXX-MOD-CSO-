...
{
cs_set_user_money(attacker, min(cs_get_user_money(attacker) + (10 * how_many_rewards), MAX_MONEY))
g_DamageDealtToHumans[attacker] -= 10 * how_many_rewards
}
else
{
cs_set_user_money(attacker, min(cs_get_user_money(attacker) + (5 * how_many_rewards), MAX_MONEY))
g_DamageDealtToHumans[attacker] -= 5 * how_many_rewards
}
}
}
}
