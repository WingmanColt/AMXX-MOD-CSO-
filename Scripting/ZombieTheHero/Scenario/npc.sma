...
new bool: g_NpcSpawn[256], bool:g_reg
new const g_NpcClassName[] = "ZOMBIE"; 
new const spawn_file[] = "%s/scenario/%s.cfg"

new Float:g_spawn[MAX_ZOMBIES][3], g_total
new g_State[256], Float:Time1
new const g_NpcModel[] = "models/player/ZB5_Regular_NEW/ZB5_Regular_NEW.mdl"
...
ExecuteHamB(Ham_TakeDamage, Victim, 0, 0, random_float(5.0, 10.0), DMG_GENERIC)
}
}
...
if(!g_reg)
{
RegisterHamFromEntity(Ham_Killed, iEnt, "fw_zb_killed")
g_reg = true
}
...
{
if(!is_valid_ent(ent2)) return;
	
emit_sound(ent2, CHAN_VOICE, g_NpcSoundDeath[2],  VOL_NORM, ATTN_NORM, 0, PITCH_NORM)
g_State[ent2] = STATE_DEATH
...
Util_PlayAnimation(ent, random_num(101,103), 1.0)	
...
if(can_see)
{
if(is_user_alive(i) && can_see_fm2(ent, i) && entity_range(ent, i) < current_dis)
{
...
public bool:can_see_fm2(entindex1, entindex2)
{
