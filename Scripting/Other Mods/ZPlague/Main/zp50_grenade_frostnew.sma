...
public logevent_round_end(id)
	...
	if(task_exists(TASK_ID2)) remove_task(TASK_ID2)	
	if(task_exists(TASK_ID3)) remove_task(TASK_ID3)	
...
frost_explode(ent)
{
	...
	static Float:Origin[3]
	pev(ent, pev_origin, Origin)  
	set_task(1.0,"create_aura", ent+TASK_ID3)
	set_task(1.0,"create_snow_fall_sprite", ent+TASK_ID2)
	set_task(15.0,"remove_ent", ent+TASK_ID)
