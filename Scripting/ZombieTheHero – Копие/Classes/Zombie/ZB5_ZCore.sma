...
static szName[32], Temp_String[64], Temp_String2[64]
new g_ForwardResult, g_Forwards[TOTAL_FORWARDS]
new g_IsZombie, g_IsAlive, g_IsConnected, g_MaxPlayers

new g_had[33][Classes], g_had2[33][EVO], g_skill[33][Skills], ef_blood[2]
new g_wpn_i, g_class_primary[10], g_class_count, g_scaned, g_skill_hud
...
if(!g_skill[id][HAD_SKILL])
{	
g_skill[id][SKILL_1] = ArrayGetCell(Class_Skill_1, g_had[id][ClassName]);
g_skill[id][SKILL_2] = ArrayGetCell(Class_Skill_2, g_had[id][ClassName]);

g_skill[id][HAD_SKILL] = true	
return
}
...
formatex(Temp_String2, sizeof(Temp_String2), "^n[Press Q] - Do Ability 2 (Ready)")
else if(g_skill[id][DO_SKILL_2])		
formatex(Temp_String2, sizeof(Temp_String2), "^n[Press Q] - Ability 2 (Reset: %i)", g_skill[id][RESET_TIME_2])
else formatex(Temp_String2, sizeof(Temp_String2), "^n[Press Q] - Ability 2 (Countdown: %i)", g_skill[id][COUNTDOWN_TIME_2])
}
else {
...
ShowSyncHudMsg(id, g_skill_hud, "%s^n%s", Temp_String, Temp_String2)
}
// MENUS
...
public native_register_zclass(const Name[], const Desc[], unlock_cost, female, Knockback, Health2, Skill_1, Skill_2)
{
param_convert(1)
param_convert(2)
...
ArrayPushCell(Class_Skill_1, Skill_1)
ArrayPushCell(Class_Skill_2, Skill_2)
